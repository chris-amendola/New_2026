...
#Frequency Analysis
col_list=info_schema_cols(table,cnxn)

#Frequency analysis query construction and execution.
logging.info('Field summarization...')
freq_query=freq_summ( table\
                         ,col_list\
                         ,by_var\
                         ,skip_list=['UNIQ_REC_ID','CUST_MED_PK_ID'])
    
#print('\n',freq_query,'\n')
var=pd.read_sql_query(freq_query,cnxn)
logging.info('Done.')

logging.info('Summary Stats Output...')
var.to_sql( table+"_fqs"
           ,sql_con
           ,if_exists="replace")
logging.info('Done.')
