...
-- Note: For duplicate detection, we'll use a hash of all columns instead of DISTINCT *
WITH row_hashes AS (
    SELECT 
        HASH(*) AS row_hash
...
SELECT 
    ...
    COUNT(DISTINCT row_hash) AS distinct_row_count,
    (COUNT(*) - COUNT(DISTINCT row_hash)) AS duplicate_row_count,
    ROUND(100.0 * (COUNT(*) - COUNT(DISTINCT row_hash)) / NULLIF(COUNT(*), 0), 2) AS duplicate_percentage
FROM row_hashes;
...
-- Using HASH(*) to check for full row duplicates
WITH row_hashes AS (
    SELECT 
        HASH(*) AS row_hash
    FROM IDENTIFIER('$target_database.$target_schema.$target_table')
...
SELECT 
    ...
    (SELECT COUNT(*) FROM IDENTIFIER('$target_database.$target_schema.$target_table')) AS total_rows,
    COUNT(DISTINCT row_hash) AS unique_rows,
    (SELECT COUNT(*) FROM IDENTIFIER('$target_database.$target_schema.$target_table')) - COUNT(DISTINCT row_hash) AS duplicate_rows,
    ROUND(100.0 * ((SELECT COUNT(*) FROM IDENTIFIER('$target_database.$target_schema.$target_table')) - COUNT(DISTINCT row_hash)) / 
          (SELECT COUNT(*) FROM IDENTIFIER('$target_database.$target_schema.$target_table')), 2) AS duplicate_percentage
FROM row_hashes;
